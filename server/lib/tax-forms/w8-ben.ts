...
export const W8BenFieldsDefinition: Partial<Record<keyof W8BenTaxFormValues, PDFFieldDefinition>> = {
  ...
  dateOfBirth: {
    ...
    transform: (value: string) => (value ? dayjs(value).format('MM/DD/YYYY') : ''),
  },
...
export const fillW8BenTaxForm = async (pdfDoc: PDFDocument, values: W8BenTaxFormValues, font: PDFFont) => {
  const form = pdfDoc.getForm();
  const signerFullName = getFullName(values.signer);

  fillPDFFormFromValues<W8BenTaxFormValues>(form, values, W8BenFieldsDefinition, font);

  // Add date & signature
  form.getTextField('topmostSubform[0].Page1[0].Date[0]').setText(dayjs().format('MM/DD/YYYY'));
  if (values.isSigned) {
    ...
    await addSignature(pdfDoc, signerFullName, {
      page: 0,

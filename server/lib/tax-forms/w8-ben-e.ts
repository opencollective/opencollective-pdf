...
export const fillW8BenETaxForm = async (pdfDoc: PDFDocument, values: W8BenETaxFormValues, font: PDFFont) => {
  const form = pdfDoc.getForm();
  const signerFullName = getFullName(values.signer);

  // Fill form
  fillPDFFormFromValues<W8BenETaxFormValues>(form, values, W8BenEFieldsDefinition, font);

  // Add date & signature
  form.getTextField('topmostSubform[0].Page8[0].f8_32[0]').setText(dayjs().format('MM/DD/YYYY'));
  if (values.isSigned) {
